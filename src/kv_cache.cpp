...
KVCache::KVCache(const ModelConfig& config)
    ...
    if (layer_positions >
        std::numeric_limits<std::size_t>::max() / width_) {
        throw std::overflow_error("KV cache dimensions overflow");
    }
...
void KVCache::Set(
    ...
    if (key.size() != width_ || value.size() != width_) {
        throw std::invalid_argument("KV vector width does not match model config");
    }
...
std::span<const float> KVCache::Key(
    std::size_t layer,
...
std::span<const float> KVCache::Value(
    std::size_t layer,
    std::size_t position) const {
    const std::size_t offset = Offset(layer, position);
