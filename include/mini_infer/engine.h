...
// 引擎以模板方式在编译期绑定具体组件（编译期多态），因此不使用任何继承或虚函数。
#pragma once
...
// 组织分词、模型前向、KV 缓存和采样的推理引擎。
// 模板参数分别是分词器、模型和采样器的具体类型，均通过“鸭子类型”约束：
//   Tokenizer 需提供 Encode / DecodeToken；
//   Model 需提供 Config / Forward；
//   Sampler 需提供 Sample。
template <typename Tokenizer, typename Model, typename Sampler>
class InferenceEngine
{
public:
    // 绑定推理所需组件并按模型配置创建 KV 缓存。
    InferenceEngine(const Tokenizer& tokenizer, const Model& model,
...
private:
    ...
    static void ValidateLogits(const Tensor& logits, std::size_t vocab_size)
    {
    ...
    static void ValidateToken(int token_id, std::size_t vocab_size)
    {
